...
if ispc
  ...
  try
    ...
    if (strcmpi(codecA,'NONE') || isempty(codecA))
      vw = videoWriter(fname, 'plugin',plugin, 'codec',codecA);
    else
    ...
    if (strcmpi(codecB,'NONE') || isempty(codecB))
      vw = videoWriter(fname, 'plugin',plugin, 'codec',codecB);
    else
